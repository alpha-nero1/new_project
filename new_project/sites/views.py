...
class View_Site(View):
    template_name = 'templates/site.html'

    def get(self, request, id):
        site = Site.objects.get(id=id)
        folders = Folder.objects.filter(user=request.user) ## send folders through for selection
        return render(request, self.template_name, {"site": site,
                                                    "folders": folders})

    def post(self, request, id): ## post reserved for votes I DO NOT LIKE THIS.
        ...
        return render(request, 'templates/votes.html', {"votes":site.votes, "comments":comments})


class Search_Sites(View):
...
## class to handle saving a site to MyLib
class Save_Site(View):

    def post(self, request, id):
        folder_name = request.POST["folder"]
        folder = Folder.objects.get(user=request.user, name=folder_name) # get users lib object
        site = Site.objects.get(id=id)
