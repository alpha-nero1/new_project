...
class Site(models.Model):
    ...
    # reference to owner, bad name its not actually an id
    owner_id = models.ForeignKey(User, on_delete=models.CASCADE)
    time_stamp = models.DateTimeField(default=timezone.now)
    site_image = models.ForeignKey(
        ThemeImage, on_delete=models.CASCADE, null=True, default=None)
...
class Comment(Write_Object):
    site = models.ForeignKey(
        Site, on_delete=models.CASCADE)  # reference to owner


class Reply(Write_Object):
    comment = models.ForeignKey(
        Comment, on_delete=models.CASCADE)  # reference to owner
